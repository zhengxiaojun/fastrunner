    ...
    <el-tabs v-model="activeName" style="margin-left: 10px;">
        <el-tab-pane label="用例列表" name="first">
            <el-container>
                <el-header style="padding: 0px; height: 50px">
                    <div class="nav-api-header">
                        <div style="padding-top: 10px; margin-left: 10px">
                            ...
                            <el-button type="danger" size="small" icon="el-icon-delete" @click="deleteNode" :disabled="buttonActivate">删除分组
                            </el-button>

                            <el-button :disabled="currentNode === '' " type="info" size="small" icon="el-icon-edit-outline"
                                @click="renameNode">重命名
                            </el-button>

                            <el-button type="primary" size="small" icon="el-icon-circle-plus-outline" @click="buttonActivate=false"
                                :disabled="buttonActivate">添加用例
                            </el-button>
                            &nbsp环境:
                            ...
                            <el-button v-if="addTestActivate" style="margin-left: 20px" type="primary" icon="el-icon-caret-right"
                                circle size="mini" @click="run = !run"></el-button>

                            <el-button v-if="addTestActivate" style="margin-left: 20px" type="danger" icon="el-icon-delete"
                                circle size="mini" @click="del = !del"></el-button>

                            <el-button :disabled="addTestActivate" type="text" style="position: absolute; right: 30px;"
                                @click="handleBackList">返回列表
                            </el-button>
                ...
                <el-container>
                    <el-aside v-show="addTestActivate">
                        <div class="nav-api-side">
                            <div class="api-tree">
                                ...
                                <el-tree @node-click="handleNodeClick" :data="dataTree" node-key="id"
                                    :default-expand-all="false" :expand-on-click-node="false" draggable
                                    highlight-current :filter-node-method="filterNode" ref="tree2" @node-drag-end="handleDragEnd">
                    ...
                    <el-main style="padding: 0;">
                        <test-list v-show="addTestActivate" :project="$route.params.id" :node="currentNode !== '' ? currentNode.id : '' "
                            :del="del" v-on:testStep="handleTestStep" :back="back" :run="run" :host="currentHost">
                        </test-list>

                        <edit-test :back="back" v-show="!addTestActivate" :project="$route.params.id" :node="currentNode.id"
                            :testStepResp="testStepResp" :config="currentConfig" :host="currentHost" v-on:addSuccess="handleBackList">
                        </edit-test>
        ...
        <el-tab-pane label="套件列表" name="second">配置管理</el-tab-pane>
    </el-tabs>
</template>
